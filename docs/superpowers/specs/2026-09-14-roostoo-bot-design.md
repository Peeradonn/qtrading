...
| 11 | 09-17 | **Tokenised stocks — Roostoo prices them around the clock; in the ranking pool they are ballast; rejected for the entry.** Roostoo fact (verified 03:33 UTC, seven hours after the US close): every stock token showed a live bid/ask (0.1–11 bp), 11 of 21 changed price within a minute, prints within ~0.2% of Bybit's xStocks → the repo's "stale at 00:00 UTC" assumption was an artefact of using the underlying share's Yahoo history. Built: `BybitSource` (24/7 hourly klines, backward paging), `TOKEN_SYMBOLS`/`token_assets()` for the 7 names Bybit lists, an `extra["live_bars"]` override so a research panel can treat stocks as tradable on carried-forward prices while volatility stays on live bars; `scripts/log_tickers.py` polling all 86 tickers every 5 min through the weekend; 228 tests. **Pre-registered study (`scripts/stocks_study.py`) vs the entry.** Main panel (20 stocks via Yahoo, assumed live, 2 years): tails and ratios better (p10 −10.3 → −6.5%, worst −23.3 → −18.5%, Sharpe 1.04 → 1.37), rally rank and total worse (top-40% in BTC-up 52 → 37%, BTC>10% 43 → 25%, total +176 → +145%). The "as recorded" run is identical to the crypto-only entry — **no earlier 00:00 UTC test ever traded a stock**. **Validation failed in magnitude:** same 7 names, same 14 months — Yahoo-assumed-live Sharpe 0.29 / median +0.12% / total +50%, Bybit tokens 1.19 / +1.55% / +116%, entry 1.30 / +1.80% / +136%; directions agree, sizes do not, so only the Bybit panel is evidence. On it the 7 tokens lower top-40% in BTC-up 63 → 56% and BTC-down 90 → 84%, Sharpe 1.30 → 1.19, Sortino 2.20 → 1.97, total +136 → +116%, and improve worst (−23.3 → −16.4%) and maxDD (−32 → −30%) — fails rules 1, 3, 4. Stocks are a tail diversifier, not a return source at the 3–14-day momentum horizon; they would suit the fallback core, not the entry. Not pursued further. SKHYB mapping wrong (token ~178 USD vs ~1.7M KRW a share); Yahoo hourly tail stuck at 2026-09-11. |
| 12 | 09-17 | **A short leg on the ranking's losers — pre-registered (`scripts/longshort_study.py`); fails the rule on rally rank only; the one rejected idea to revisit.** Prompted by the user asking whether "long-only" was evidence or assumption: the BTC hedge had been through the harness (row 8), the losers' short leg only through a daily spike. Code: `short_k / short_share / short_buffer_rank / short_negative_only` (bottom of the same ranking, hysteresis from the bottom, signed weights net inside the constant-correlation vol target, long hysteresis ignores short holdings); 236 tests; the entry's equity is unchanged. **Run 1 vs the entry, in-sample:** the leg alone loses (−24%; post-May −19%; worst −35%). At 25% of gross: median +1.38 → +2.17%, p10 −10.3 → −7.0%, worst −23.3 → −18.3%, median 14-day drawdown 6.8 → 4.6% (the core's), P(fortnight < −10%) 10.7 → 4.9%, Sharpe 1.04 → 2.08, Sortino 1.68 → 3.85, Calmar 0.20 → 0.50, total +176 → +222%, maxDD −47 → −33%; post-May Sharpe 1.59 → 2.16, total +35.5 → +31.0%. Cost: top-40% in BTC-up 52 → 44% (post-May 45 → 33%); BTC>10% 43 → 31%. Rule 4 fails by 5 points; everything else passes. 50% share: in-sample Sharpe 2.83, post-May 0.81 — not robust, as the spike had shown. It dominates the *core* on every column including rally rank (44 vs 40). Screen 2 pass probability vs the entry by field size: N=30 83 vs 88%, 50: 64 vs 67%, 60: 56 = 56%, 80: 46 vs 41%, 100: 37 vs 29%. **Battery (post-hoc, labelled):** share 15/25/35% is a smooth dial (Sharpe 1.38/2.08/2.43, p10 −8.6/−7.0/−5.1%, up-rank 47/44/39%); 4/6/8 shorts all work (Sharpe 1.67/2.08/1.80); K=8 longs 1.28 (the entry at K=8: 0.80); **12:00 selection hour: tails still better (p10 −9.4 → −6.5%, worst −22.2 → −15.8%) but Sharpe only 1.30 → 1.39** → the tail gain is robust, the ratio doubling is partly hour luck; a higher target does not buy back rally rank (vt4/vt5: up-rank 45/46%) because the 1x gross cap binds (long 63 → 73%). **Why it works where the BTC hedge did not:** losers carry negative alpha as well as beta. **Why it is not adopted:** every number assumes a short is a SELL beyond the holding, valued at price, no borrow cost, gross ≤ 1x with alts shortable — the API documents describe none of it; realised gross drifts to 1.16×; ~3% of equity per shorted name is exposed to a squeeze. **Path:** long-only entry stays the default; build engine short support behind a flag and paper-trade ls25 during the waiting week; on Sep 26 place one real short on the test account; adopt only if the mechanics match the assumptions. |
| 13 | 09-17 | **Engine short support, behind `allow_short` (default off; no shipped competition config sets it).** Config flag → `reconcile(allow_short=)` treats a negative balance worth more than dust as a short position valued negatively (without the flag a negative balance is only ever rounding, as the FAQ says) → the shared `plan_orders(allow_short=)` may sell beyond the holding and covers a short with a BUY → `PaperExchange(allow_short=)` lets a sell leave a negative balance so the path can be paper-traded. Strategy `gross_guard` (1.0): a book with shorts whose realised gross drifts past the cap is rebalanced to target at once, drift band or not. **Validated in the simulator:** the guard alone churns (2,370 → 5,530 trades) because targets sit at the cap; with `max_exposure = 0.9` plus the guard, realised gross peaks at 1.02 and is ≤ 0.96 in 99% of hours (as studied: 1.16 / 1.04), trades +12%, results essentially unchanged (median +2.18%, p10 −6.6%, worst −17.1%, Sharpe 1.98, total +196%). That is `configs/paper-ls25.toml`, started under the supervisor on its own cache copy. 242 tests, including a loop cycle that opens a short, reconciles it as a holding and does not short twice, and one proving a negative target places nothing while the flag is off. **Adoption rule, fixed now:** on the test account one small SELL of an alt not held must (1) fill, (2) appear as a negative balance, (3) reduce portfolio value by price moves one-for-one, (4) carry no borrow charge over 24 h, and (5) a BUY must close it; and the 1x limit must reject exposure-increasing orders rather than liquidate. All five, or the entry stays long-only. |
...
**Answered by the official FAQ (2026-09-17):** rate limit 30 calls/min across all endpoints (the client now paces at 2 s, so a 12-order cycle takes ~30 s and cannot exceed it even with retries) · Roostoo's real-time prices are streamed from Binance · two key sets arrive by Sep 26, a test account and the official one, so the signed order path is rehearsed on the test account before the 30th · the first trade must execute by Oct 1 8pm (the first cycle on a fresh account buys the book, so this is automatic) · mid-competition changes are allowed with clear commits, but manual intervention *including stopping the bot by hand* is prohibited — `mode = "hold"` is reserved for a genuine fault, with the reason in the commit · the system liquidates at the end · long and short are both allowed (run 8: not used) · the leaderboard shows returns and ranking in real time, so the team count per region is visible on Sep 30 · small negative balances after sells are rounding and harmless (reconcile ignores holdings below a dust threshold) · EC2 is Sydney only.

**Still open:** ratio sampling frequency, annualisation and cross-team normalisation · definition of "enough trades" on an active day · limit-order fill model · portfolio valuation price and final snapshot time · weekend price source for tokenized stocks (being observed by `scripts/log_tickers.py`; stocks are not in the entry) · how a short is placed and valued through the API (not needed).
